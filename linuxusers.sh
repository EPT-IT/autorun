...
if [[ $inputuid =~ ^[[:digit:]]+$ ]];then
sudo useradd --create-home --user-group --groups adm,cdrom,sudo,dip,plugdev,lpadmin,lxd,sambashare --uid $inputuid --shell /bin/bash --comment "$inputfullname" $username
echo -e "${ORANGE}User ${GREEN}$username ${ORANGE}has been created for ${GREEN}$inputfullname ${ORANGE}with the UID ${GREEN}$inputuid${NC}"
else
sudo useradd --create-home --user-group --groups adm,cdrom,sudo,dip,plugdev,lpadmin,lxd,sambashare --shell /bin/bash --comment "$inputfullname" $username
...
# Update
echo ""
echo -e "${ORANGE}Would you like to update and install the carepackage?${NC}"
read -r -p "[Y/n] " input
